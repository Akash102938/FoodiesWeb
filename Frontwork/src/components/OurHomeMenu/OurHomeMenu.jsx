...
const categories = ['Breakfast', 'Lunch', 'Dinner', 'Italian', 'Desserts', 'Drinks']
    ...
    px-4 sm:px-6 lg:px-8'>
        <div className='max-w-7xl mx-auto'>
            ...
            <div className='flex flex-wrap justify-center gap-4 mb-16'>
                {categories.map(cat =>(
                    <button key={cat} onClick={()=>{setActiveCartegory(cat)}}
                    className={`px-4 sm:px-6 py-2 rounded-full border-2 transition-all duration-300 transform
                    font-cinzel text-sm sm:text-lg tracking-widest bacdrop-blur-sm
                    ${activeCategory === cat ? 
                        'bg-gradient-to-r from-amber-900/80 to-amber-700/80 border-amber-800 scale-105 shadow-xl shadow-amber-900/30'
                        : 'bg-amber-900/20 boder-amber-800/30 text-amber-100/80 hover:bg-amber-800/40 hover:scale-95 '
                    }`}>
                      {cat}
                    </button>
                ))}
            </div>
            <div className='grid gird-cols-1 gap-8 sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-3 xl:grid-cols-4 '>
