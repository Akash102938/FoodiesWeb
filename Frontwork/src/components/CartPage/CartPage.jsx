    ...
    to-[#3e2b1d]'>
      <div className='max-w-7xl mx-auto'>
          <h1 className='text-4xl sm:text-5xl md:text-6xl font-bold text-center mb-12 animate-fade-in-down'>
            <span className='font-dancingscript block text-5xl sm:text-6xl md:text-7xl mb-2 bg-gradient-to-r from-amber-500
          ...
          {cartItems.length === 0 ? (
            ...
            <>
              <div className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6'>
                {cartItems.map((item)=>(
                  <div key={item.id}
                   className='group bg-amber-900/20 p-4 rounded-xl border-4 border-dashed
                   ...
                   transform hover:-translate-y-1 animate-fade-in'>
                      ...
                      onClick={()=> setSelectedImage(item.image)}>
                            ...
                            object-contain' onError={(e)=> { if (e?.currentTarget) e.currentTarget.src = fallbackImage }} />
                      </div>
              ...
              <div className='mt-12 pt-8 border-t boder-amber-800/30 animate-fade-in-up'>
               <div className='flex flex-col sm:flex-row justify-between items-center gap-8'>
                ...
                <div  className='flex items-center gap-8'>
                       ...
                       <button className='b-amber-900/40 px-8 py-3 rounded-full font-cinzel uppercase
      ...
      {selectImage && (
        <div className='fixed inset-0 z-50 flex items-center justify-center bg-amber-900/40 bg-opacity-75 backdrop-blur-xl
        p-4 overflow-auto'
        onClick={()=> setSelectedImage(null)}>
           <div className='relative max-w-full max-h-full'>
             <img src={selectImage} alt='full view' className='max-w-[90vw] max-h-[90vh] rounded-lg object-contain'/>
             <button onClick={()=> setSelectedImage(null)}
              className='absolute top-1 right-1 bg-amber-900/80 rounded-full p-2 text-black hover:bg-amber-800 duration-200 active:scale-95'>
