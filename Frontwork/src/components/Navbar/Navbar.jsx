...
function Navbar() {
  ...
  return (
    <nav className="w-full bg-[#2D1B0E] border-b-8 border-amber-900/30 sticky top-0 z-50 shadow-[0_25px_50px_-12px_rgba(120,53,15,0.3)] font-vibes group overflow-x-hidden">


      {/* Decorative Line */}
      <div className="absolute -top-3 left-1/2 -translate-x-1/2 w-full max-w-7xl px-4">
        <div className="h-[6px] bg-gradient-to-r from-transparent via-amber-600/50 to-transparent shadow-[0_0_20px_rgba(245,158,11,0.3)]">
        ...
        <div className='flex justify-between items-center h-16 md:h-20 lg:h-24'>
          ...
          <div className='flex-shrink-0 flex items-center space-x-2 group relative md:-translate-x-4 lg:-translate-x-6 ml-0 md:ml-2'>
            ...
            <div className="flex flex-col relative ml-2 max-w-[140px] md:max-w-[160px] lg:max-w-none">
              <NavLink
                to="/"
                className='text-2xl md:text-xl lg:text-4xl bg-gradient-to-r from-amber-400 to-amber-600 bg-clip-text text-transparent font-monsieur tracking-wider drop-shadow-[0_2px_2px_black] drop-shadow-black -translate-x-2 truncate
                md:truncate-none'
              >
                Foodie-Frenzy
              </NavLink>
          ...
          <div className="hidden md:flex items-center space-x-2 lg:space-x-4 flex-1  justify-end">
            {navLinks.map((link) => (
              <NavLink
                ...
                to={link.to}
                className={({ isActive }) =>
                  `group  px-3 md:px-3  lg:px-4 py-2 md:py-2 text-sm md:text-[15px] lg:text-base relative transition-all duration-300 flex items-center hover:bg-amber-900/20 rounded-3xl border-2 ${isActive
